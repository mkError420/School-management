...
const schema = z.object({
  ...
  lasttName: z.string() .min(1, { message: 'Last name is required!'}),
...
const TeacherForm = ({
  ...
  return <form className='flex flex-col gap-8' onSubmit={onSubmit}>
    ...
    <input type="text" {...register("username")} className="ring-[1.5px] ring-gray-600 p-2 rounded-md text-sm w-full " />
    {errors.username?.message && (<p className="text-xs text-red-700">{errors.username?.message.toString()}</p>
  )}
  </div>
